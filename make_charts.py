...
TEST_PRED_PATH = os.path.join(HERE, "test_predictions.csv")
...
def train_and_evaluate(train_df, split_season=2022, fast=False):
    ...
    # Save actual-vs-predicted rows for the held-out test set (best model +
    # baseline), for the actual-vs-forecast chart.
    pred_df = train_df.loc[test_mask, ["player", "team", "season", "league", "primary_position"]].copy()
    pred_df["actual_next_composite"] = y_test.values
    pred_df["predicted_next_composite"] = test_preds[best_name]
    pred_df["baseline_predicted"] = baseline_pred_all.values
    pred_df["model_used"] = best_name
    pred_df.to_csv(TEST_PRED_PATH, index=False)
    print(f"Saved held-out test-set predictions -> {TEST_PRED_PATH}")
