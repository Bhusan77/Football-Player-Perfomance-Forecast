...
# Credentials intentionally removed for the standalone bundle. All data access
# is redirected to a local CSV-backed in-memory SQLite store by api_server.py,
# which overrides get_connection() below, so this is never used.
...
# Metrics retired 2026-07: SCA/GCA + deep-possession (progression, touches-by-
# zone, carries). FBref's Opta feed ended 2026-01-20, leaving these with no free
# data source, so any score built on them would be misleading/partial. They are
# stripped from the style categories below - which cascades to the composite
# index and similar-players search (both consume get_playing_style_categories()).
# To un-retire (e.g. after an Opta licence), empty this set.
...
# 2025-26 has no free full-season source for these detail metrics (they are NULLed
# in the wide table by harmonize_xg.py's NULL_ORPHANS/NULL_GK). We exclude them from
# the radar so it only ever shows real, available data points - no empty axes. Must
# stay in sync with harmonize_xg.py's NULL lists (which imports these two sets).
...
# Features are pool-independent (no z-score/composite) so there's no
# train/inference drift and training is fast.
...
# Role/volume descriptors - these measure how OFTEN a player does something
# (taking corners, racking up touches/distance), not how WELL. They must not
# be surfaced as the headline reason a category is a strength/weakness, since
# being the designated set-piece taker isn't a quality signal.
DESCRIPTOR_METRICS = {
...
# The style categories are the decision criteria (all benefit-type: higher percentile
# = better). AHP turns a position's importance priorities into a consistent weight
# vector; TOPSIS ranks the target + shortlist by closeness to the ideal; sensitivity
# checks whether that ranking survives weight shifts. Runs on league data only.
_AHP_PRIORITY = {
...
# Saaty random-consistency index by matrix size (for the consistency ratio).
...
# ------------------------------------------------------------------------------
# Background job state for run_update.
# Holds a single rolling job per server process; new run_update overwrites it.
# Step-level progress + a final summary land here for cmd_get_update_progress.
# ------------------------------------------------------------------------------
import threading
